...
<div class="max-w-4xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
    <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200">Edit Service</h2>
    ...
    <form action="{{ route('services.update', $service->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="4"
                class="w-full p-2 border rounded dark:bg-gray-700 dark:text-white" required>{{ old('description', $service->description) }}</textarea>
            @error('description')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
